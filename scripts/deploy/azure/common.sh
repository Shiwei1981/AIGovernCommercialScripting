...
RUNTIME_REQUIRED_VARS=(
  ...
  QUERYVISIBILITY_API_URL
  QUERYVISIBILITY_OPENAPI_URL
  QUERYVISIBILITY_OPENAPI_PATH
  MASK_API_URL
  MASK_OPENAPI_URL
  MASK_OPENAPI_PATH
)
...
load_env_file() {
  ...
  export DEPLOY_DB_READ_GOVERNANCE_ENABLED
  DEPLOY_DB_READ_GOVERNANCE_ENABLED="$(resolve_value PROD_DB_READ_GOVERNANCE_ENABLED DB_READ_GOVERNANCE_ENABLED true)"
...
assert_prod_safe_flags() {
  ...
  if [[ "${DEPLOY_DB_READ_GOVERNANCE_ENABLED,,}" != "true" ]]; then
    fail "PROD_DB_READ_GOVERNANCE_ENABLED/DB_READ_GOVERNANCE_ENABLED must be true for production deployment."
  fi
}
