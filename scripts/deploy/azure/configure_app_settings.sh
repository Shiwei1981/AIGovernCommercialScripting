...
if [[ -n "${DEPLOY_APPLICATIONINSIGHTS_CONNECTION_STRING:-}" ]]; then
  app_settings+=("APPLICATIONINSIGHTS_CONNECTION_STRING=$DEPLOY_APPLICATIONINSIGHTS_CONNECTION_STRING")
fi

if [[ -n "${DEPLOY_OTEL_SERVICE_NAME:-}" ]]; then
  app_settings+=("OTEL_SERVICE_NAME=$DEPLOY_OTEL_SERVICE_NAME")
fi
